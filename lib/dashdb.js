...
DASHDB.prototype.testConnection = function(conn) {
 ...
 *
 */
DASHDB.prototype.executeSQL = function(sql, params, options, callback) {
  debug('DASHDB.prototype.executeSQL (enter)',
        sql, params, options);
  var self = this;
  ...
  var limit = 0;
  var offset = 0;
  // This is standard DASHDB syntax. LIMIT and OFFSET
 ...
 * DASHDB requires double quotes for case-sensitivity
 *
 * @param {string} name A database identifier
 * @returns {string} The escaped database identifier
 */
DASHDB.prototype.escapeName = function(name) {
  debug('DASHDB.prototype.escapeName name=%j', name);
  if (!name) return name;
  name.replace(/["]/g, '""');
  return '"' + name + '"';
};

function dateToDASHDB(val) {
 ...
 */
DASHDB.prototype.toColumnValue = function(prop, val) {
  debug('DASHDB.prototype.toColumnValue prop=%j val=%j', prop, val);
  if (val === null) {
 ...
 */
DASHDB.prototype.fromColumnValue = function(prop, val) {
  debug('DASHDB.prototype.fromColumnValue %j %j', prop, val);
  if (val === null || !prop) {
    return val;
  }
  switch (prop.type.name) {
 ...
 */
DASHDB.prototype.getPlaceholderForIdentifier = function(key) {
 ...
 */
DASHDB.prototype.getPlaceholderForValue = function(key) {
  debug('DASHDB.prototype.getPlaceholderForValue key=%j', key);
  return '(?)';
};
 ...
 */
DASHDB.prototype.buildInsertDefaultValues = function(model) {
 ...
 */
DASHDB.prototype.createTable = function(model, options, cb) {
  debug('DASHDB.prototype.createTable ', model, options);
  cb();
};

DASHDB.prototype.buildIndex = function(model, property) {
  debug('DASHDB.prototype.buildIndex %j %j', model, property);
};

DASHDB.prototype.buildIndexes = function(model) {
  debug('DASHDB.prototype.buildIndexes %j', model);
};
 ...
 * @param {Function} [callback] The callback function
 */
DASHDB.prototype.updateOrCreate = DASHDB.prototype.save =
...
DASHDB.prototype.applyPagination = function(model, stmt, filter) {
  debug('DASHDB.prototype.applyPagination');
  var limitClause = buildLimit(filter.limit, filter.offset || filter.skip);
  return stmt.merge(limitClause);
};


DASHDB.prototype.getCountForAffectedRows = function(model, info) {
 ...
 * @param {Function} [cb] The callback function
 */
DASHDB.prototype.dropTable = function(model, cb) {
  var self = this;
...
DASHDB.prototype.buildColumnDefinitions = function(model) {
  var self = this;
  var sql = [];
...
DASHDB.prototype.buildColumnDefinition = function(model, prop) {
...
DASHDB.prototype.columnDataType = function(model, property) {
...
DASHDB.prototype.buildColumnType = function buildColumnType(propertyDefinition) {
  var self = this;
  var dt = '';
  ...
  debug('DASHDB.prototype.buildColumnType %j %j', p.type.name, dt);
  return dt;
};

DASHDB.prototype.convertTextType = function convertTextType(p, defaultType) {
  var self = this;
  var dt = defaultType;
  var len = p.length ||
...
DASHDB.prototype.convertNumberType = function convertNumberType(p, defaultType) {
  var self = this;
  var dt = defaultType;
